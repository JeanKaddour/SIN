...
def get_activation(name: str, leaky_relu: Optional[float] = .5) -> nn.Module:
    if name == 'leaky_relu':
...
def get_optimizer(args: Namespace, params: Iterator[nn.Parameter], net: Optional[str] = None) -> optim.Optimizer:
...
def get_lr_scheduler(args: Namespace, optimizer: optim.Optimizer) -> Union[
    optim.lr_scheduler.ExponentialLR, optim.lr_scheduler.CosineAnnealingLR, optim.lr_scheduler.CyclicLR, NoneScheduler]:
...
def get_optimizer_scheduler(args: Namespace, model: nn.Module, net: Optional[str] = None):
