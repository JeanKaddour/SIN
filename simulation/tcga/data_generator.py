...
from simulation.data_generator import DataGenerator, get_treatment_ids, get_treatment_propensities
...
class TCGADataGenerator(DataGenerator):
    ...
    def generate_train_data(self) -> None:
        treatment_ids = self.get_train_assignments(units=self.in_sample_dataset.get_units()['features'])
    ...
    def generate_test_units(self, test_units, test_assignments):
        ...
        for i in range(len(test_units['ids'])):
            ...
            test_unit = TestUnit(covariates=test_units['features'][i], treatment_ids=test_assignments_ids[i],
    ...
    def generate_test_data(self):
        ...
        in_sample_treatment_assignments = self.get_test_assignments(units=in_sample_units['features'],
                                                                    mode='most',
                                                                    num_test_treatments_per_unit=self.args.max_test_assignments)
        out_sample_treatment_assignments = self.get_test_assignments(units=out_sample_units['features'],
                                                                     mode='most',
                                                                     num_test_treatments_per_unit=self.args.max_test_assignments)
