...
def generate_id_to_graph_dict_sw(args):
    treatments_list = generate_sw_graphs(num_graphs=args.num_graphs, min_nodes=args.min_num_nodes,
    ...
    for treatment in treatments_list:
        id = treatment['id']
        id_to_graph_dict[id] = {'c_size': len(treatment['node_features']),
                                'node_features': treatment['node_features'],
                                'edges': treatment['graph'], 'graph_features': treatment['graph_features'],
                                'one_hot_encoding': one_of_k_encoding(x=id, allowable_set=all_treatment_ids)}
...
def generate_id_to_graph_dict_tcga(args):
    ...
    for i, raw_molecule in enumerate(tqdm(raw_data)):
        c_size, features, edge_index, edge_types = smiles_to_graph(raw_molecule['smiles'])
        id_to_graph_dict[i] = {'c_size': c_size,
                               'node_features': features,
                               'edge_types': edge_types,
                               'edges': edge_index, 'prop': raw_molecule['prop'], 'qed': raw_molecule['qed'],
